...
object MyModule {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(formatResult("fib", 0, fib))
    println(formatResult("fib", 1, fib))
    println(formatResult("fib", 2, fib))
  ...
  def fib(n: Int): Int = {
    if (n == 0) 0
      ...
      @annotation.tailrec
      def go(n: Int, prevPrev: Int, prev: Int): Int = {
        val curr = prevPrev + prev
        if (n == 0) curr
        else go(n-1, prev, curr)
      }
      go(n - 2, 0, 1)
    }
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
      ...
      def go(i: Int): Boolean = {
        if (i > as.length - 2) true
        else gt(as(i), as(i+1)) && go(1 + i)
      }
      go(0)
  }
...
object TestPolymorphic {
  ...
  def formatResult[A](a: Array[A], f: Array[A] => Boolean) = {
    val s = f(a)
    println(s"Array: ${a.mkString(" ")} is sorted: $s")
  }

  def main(args: Array[String]): Unit = {
    val s = (a: Array[Int]) => isSorted(a, (x: Int, y: Int) => x > y)
    formatResult(Array(3, 2, 1), s)
    formatResult(Array(3, 4 ,1), s)
    formatResult(Array(1), s)
    formatResult(Array(), s)
  }
}
